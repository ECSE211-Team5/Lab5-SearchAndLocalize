...
public class Lab5 {
  ...
  // initialize multiple light ports and EV3 brick display in main
  ...
  /**
   * This variable denotes the radius of our wheels in cm. Used only during the localization phase
   * in our main method.
   */
  ...
  /**
   * This variable denotes the track distance between the center of the wheels in cm (measured and
   * adjusted based on trial and error). Used only during the localization phase in our main method.
   */
  ...
  public static void main(String[] args) throws OdometerExceptions {
    ...
    while (Button.waitForAnyPress() != Button.ID_ESCAPE);
    ((EV3UltrasonicSensor) usSensor).close();
    System.exit(0);
  }
      ...
      if (counter % 2 == 0) {
        ...
        for (double j = URy - 1 + 0.3; j > LLy; j--) {
          ...
          if (searcher.search(110, targetColor))
            return;
          navigation.turnTo(180, false);
          if (searcher.search(250, targetColor))
            return;
          navigation.turnTo(180, false);
        }
      }
